function [C,CM,DISTTOCM,NRCLRET] = kmeansprun_ext_merge6(A,MAXNRPOINT,EXTTRESH,QUALCRIT,RAND,PLOT,DIM,A_ORG)

% [C,CM,DISTTOCM,NRCLRET] = kmeansprun_ext_merge6(A,MAXNRPOINT,EXTTRESH,QUALCRIT,RAND,PLOT,DIM,A_ORG)
...
for loop=1:MAXITER
   ...
   if WAIT >= STARTPRUN_EXT
      ...
      for l1=1:NUMCLUS
         for l2=find(dist_misval(CMI(:,l1+1:NUMCLUS),CMI(:,l1),QUALCRIT)<2*EXTTRESH)+l1
      ...
      while min(min(DM)) <= EXTTRESH
         ...
         DM(C2,:)=[];
         DM(:,C2)=[];
         DM(C1,C1+1:NUMCLUS)=inf;
         for l1=find(dist_misval(CMI(:,C1+1:NUMCLUS),CMI(:,C1),QUALCRIT)<2*EXTTRESH)+C1
         ...
         DM(1:C1-1,C1)=inf;
         for l1=find(dist_misval(CMI(:,1:C1-1),CMI(:,C1),QUALCRIT)<2*EXTTRESH)
            ACL=A(:,find(CI==l1|CI==C1));
            CLSAM = size(ACL,2);
            if CLSAM > 0
